...
def audit_gold(root: Path, langs: list[str], split: str, index_names: list[str] | None = None) -> dict:
    known: set[str] = set()
    names = index_names or [d.name for d in sorted(root.iterdir()) if d.is_dir() and (d / "hnsw.bin").exists()]
    for name in names:
        ix = ChunkIndex.load(root, name)
...
def main() -> None:
    ...
    built = [d.name for d in sorted(root.iterdir()) if d.is_dir() and (d / "hnsw.bin").exists()]
    if not built:
        print(f"No indexes found in {root}")
        return
    per_index = [audit_index(root, n) for n in built]
    gold = audit_gold(root, args.langs, args.split, built)
